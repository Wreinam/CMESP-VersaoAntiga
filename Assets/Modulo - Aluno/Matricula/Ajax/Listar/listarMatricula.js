function listarMatricula() {
    $.ajax({
        url: "../Matricula/PDO/Listar/listarMatricula.php",
        method: 'POST',
        dataType: 'json'
    }).done(function (result) {
        $("#matriculas_div").empty();
        result.forEach((item) => {
            $("#matriculas_div").append(`

            <div class="card text-bg-light mb-3" style="max-width: 18rem; min-width: 15rem;">
                <div class="card-header pb-0 d-flex justify-content-between">
                  <p class="fs-5">${item["nome_modalidade"]}</p>
                  <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#cancelarMatriculaModal${item["id_turma"]}" aria-label="Close" style="height: 40px">Cancelar</button>
                </div>
                <div class="modal fade" id="cancelarMatriculaModal${item["id_turma"]}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        <button type="button" onclick="excluirMatricula(${item["id_matricula"]}, ${item["id_turma"]})" class="btn btn-danger" data-bs-dismiss="modal">Sim</button>
                ...
                <div class="card-body">
                  <p class="card-text mb-2 fs-6">Status: ${item["status"]}</p>
                  <p class="card-text mb-2 fs-6">Faltas: ${item["quant_faltas"]}</p>
